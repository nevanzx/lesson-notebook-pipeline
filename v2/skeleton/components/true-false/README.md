# true-false
Hard, situational self-check bank (Section 7). Each statement tempts a wrong near-miss
answer; click TRUE/FALSE locks the item (no retry), colours the picked button, dims the
other, and reveals the explanation naming the trap. Scoreboard denominator is the TOTAL.

Data (`LN.data.tf7 = ...`):
```js
{ items: [{ s: "If a venture's BEP is 500 but only 400 are obtainable, the fix is to lower price.",
            a: false, e: "Lowering price raises BEP. The fix is cutting fixed or variable cost." }] }
```
When-not: trivia recall; anything graded (this format refuses to grade).
